...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS study_topic_attachments (
    id TEXT PRIMARY KEY,
    ...
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (study_topic_id) REFERENCES study_topics(id) ON DELETE CASCADE
  );
`);
...
async function startServer() {
  ...
  app.delete('/api/admin/students/:id', authenticateToken, requireAdmin, (req: any, res) => {
  ...
  app.delete('/api/admin/students/cleanup', authenticateToken, requireAdmin, (req: any, res) => {
    // Delete students whose email domain is not 'mkce'
    const studentsToDelete = db.prepare(`
      SELECT id FROM users 
      WHERE role = 'student' AND email NOT LIKE '%@mkce.%'
    `).all();

    let deletedCount = 0;
    for (const student of studentsToDelete) {
      db.prepare('DELETE FROM submissions WHERE student_id = ?').run(student.id);
      db.prepare("DELETE FROM users WHERE id = ? AND role = 'student'").run(student.id);
      deletedCount++;
    }
  ...
  app.post('/api/study-topics', authenticateToken, upload.array('attachments', 10), (req: any, res) => {
    ...
    if (!title || (!content && (!req.files || req.files.length === 0))) {
      return res.status(400).json({ error: 'Title and content or attachments are required' });
    }
    ...
    const studyTopicId = result.lastInsertRowid;
    ...
    if (req.files && Array.isArray(req.files)) {
      for (const file of req.files) {
        ...
        db.prepare(
          `INSERT INTO study_topic_attachments (id, study_topic_id, filename, original_name, mime_type, size)
           VALUES (?, ?, ?, ?, ?, ?)`
        ).run(attachmentId, studyTopicId, file.filename, file.originalname, file.mimetype, file.size);
  ...
  app.get('/api/study-topics', authenticateToken, (req: any, res) => {
    if (req.user.role === 'faculty') {
      ...
      // Add attachments to each topic
      for (const topic of topics) {
        const attachments = db.prepare(
          `SELECT id, filename, original_name, mime_type as mimeType, size
           FROM study_topic_attachments
           WHERE study_topic_id = ?
           ORDER BY created_at ASC`
        ).all(topic.id);

        topic.attachments = attachments.map((att: any) => ({
          ...att,
  ...
  app.delete('/api/study-topics/:id', authenticateToken, (req: any, res) => {
    ...
    const attachments = db.prepare('SELECT filename FROM study_topic_attachments WHERE study_topic_id = ?').all(topicId);
    ...
    if (result.changes > 0) {
      ...
      for (const attachment of attachments) {
        try {
          fs.unlinkSync(path.join(uploadsDir, attachment.filename));
        } catch (err) {
  ...
  app.delete("/api/quizzes/:id", authenticateToken, (req: any, res) => {
    console.log('Delete quiz request from user:', req.user?.name, 'role:', req.user?.role);
    ...
    let deleteQuery = "DELETE FROM quizzes WHERE id = ?";
    let params = [quizId];
    
    if (req.user.role === 'faculty') {
      deleteQuery += " AND faculty_id = ?";
      params.push(req.user.id);
    }
    
    console.log('Executing delete query:', deleteQuery, 'params:', params);
